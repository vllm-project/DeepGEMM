...
static void sm100_nvfp4_mega_moe(
    ...
    // Heuristics
    const auto config = get_nvfp4_mega_moe_config(
        num_ranks, num_experts, num_experts_per_rank,
        num_max_tokens_per_rank, num_tokens, num_topk, hidden, intermediate_hidden,
        num_ring_tokens, num_sf_ring_tokens,
        shared_bf16 and shared_full_k);
    const int shared_block_k = config.block_k / (shared_bf16 and not shared_full_k ? 2 : 1);
    ...
    const auto tensor_map_l1_acts = make_tma_2d_desc(l1_acts,
                                                     ...
                                                     config.block_k / 2, 0, false, false);
    ...
    // Post-SwiGLU output has 64 logical FP4 values per tile: a 32-byte swizzle.
    ...
    const auto tensor_map_shared_l1_acts_sf = num_shared_experts > 0 and not shared_bf16 ? make_tma_sf_desc(
        cute::UMMA::Major::MN, shared_l1_acts_sf,
        static_cast<int>(shared_l1_acts_sf.size(0)), hidden,
        config.sf_block_m, 32,
        1, 0, 0, false,
        config.block_k / 128) : tensor_map_l1_acts_sf;
    ...
    const auto tensor_map_shared_l1_weights_sf = num_shared_experts > 0 and not shared_bf16 ? make_tma_sf_desc(
        cute::UMMA::Major::MN, shared_l1_weights_sf,
        shared_intermediate_hidden * 2, hidden,
        config.block_n, 32,
        1, 0, 0, false,
        config.block_k / 128) : tensor_map_l1_weights_sf;
    const auto tensor_map_shared_l1_output = num_shared_experts > 0 ? make_tma_2d_desc(
        ...
        config.swizzle_acts_mode / (shared_bf16 ? 1 : 2)) : tensor_map_l1_output;
    ...
    const auto tensor_map_shared_l2_acts_sf = num_shared_experts > 0 and not shared_bf16 ? make_tma_sf_desc(
        cute::UMMA::Major::MN, shared_l2_acts_sf,
        static_cast<int>(shared_l2_acts_sf.size(0)), shared_intermediate_hidden,
        config.sf_block_m, 32,
        1, 0, 0, false,
        config.block_k / 128) : tensor_map_l2_acts_sf;
    ...
    const auto tensor_map_shared_l2_weights_sf = num_shared_experts > 0 and not shared_bf16 ? make_tma_sf_desc(
        cute::UMMA::Major::MN, shared_l2_weights_sf,
        hidden, shared_intermediate_hidden,
        config.block_n, 32,
        1, 0, 0, false,
        config.block_k / 128) : tensor_map_l2_weights_sf;
    ...
    // Compile
    const auto kernel = jit->compile("sm100_nvfp4_mega_moe", std::format(R"(
#include <deep_gemm/impls/sm100_nvfp4_mega_moe.cuh>

using namespace deep_gemm;

static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(&sm100_nvfp4_mega_moe_impl<
        {},
        {}, {},
        {}, {},
        {}, {}, {},
