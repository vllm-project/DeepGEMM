...
#include <deep_gemm/ptx/nvfp4.cuh>
...
sm100_nvfp4_mega_moe_impl(void* y,
                            int* cumulative_local_expert_recv_stats,
                            const float* l1_alpha,
                            const float* l2_alpha,
    ...
    const auto buffer = layout::NVFP4MegaMoEBuffer(
    ...
    // Data types
    // Routed operands use packed E2M1/per-16 E4M3; shared operands use MXFP8 or BF16.
    using a_dtype_t = shared_dtype_t;
    using routed_a_dtype_t = cutlass::float_e2m1_t;
    using shared_b_dtype_t = shared_dtype_t;
    DG_STATIC_ASSERT(cute::is_same_v<weight_dtype_t, cutlass::float_e2m1_t>, "NVFP4 requires packed E2M1 weights");
    DG_STATIC_ASSERT(SHARED_BLOCK_K * sizeof(shared_dtype_t) >= BLOCK_K / 2, "Shared storage must fit packed routed operands");
    ...
    constexpr uint32_t UMMA_BLOCK_K = kSharedBF16 ? 64 : 128;
    constexpr uint32_t UMMA_K = kSharedBF16 ? 16 : 32;
    ...
    // Shared memory sizes
    // L1 reserves the larger shared-expert output (MXFP8 or BF16).
    // L2 stages BF16 combine writes in the same storage.
    ...
    if (warp_idx < kNumDispatchWarps) {
        ...
        for (uint32_t token_idx = sm_idx * kNumDispatchWarps + warp_idx; ; token_idx += kNumGlobalWarps) {
            ...
            constexpr uint32_t kNumChunks = (kHidden / 2) / kNumBytesPerPull;
            DG_STATIC_ASSERT(kNumChunks * kNumBytesPerPull == kHidden / 2, "kNumBytesPerPull must divide packed hidden");
        ...
        while (scheduler.get_next_task(task_info)) {
            ...
            for (uint32_t k_block_idx = 0; k_block_idx < num_k_blocks; advance_pipeline(k_block_idx)) {
                ...
                if (cute::elect_one_sync()) {
                    if (task_info.is_shared()) {
                        tma::copy<SHARED_BLOCK_K, LOAD_BLOCK_N, kSwizzleBMode, shared_b_dtype_t>(
        ...
        if (is_leader_cta) {
            ...
            auto routed_instr_desc = cute::UMMA::make_instr_desc_block_scaled<
                    weight_dtype_t, routed_a_dtype_t, float, cutlass::float_ue4m3_t,
                    UMMA_M, UMMA_N,
            ...
            auto shared_instr_desc = cute::UMMA::make_instr_desc_block_scaled<
                ...
                cute::UMMA::Major::K, cute::UMMA::Major::K
            >();
            auto shared_bf16_instr_desc = cute::UMMA::make_instr_desc<
            ...
            auto routed_b_desc = mma::sm100::make_umma_desc<cute::UMMA::Major::K, LOAD_BLOCK_N, BLOCK_K, BLOCK_K / 2>(reinterpret_cast<weight_dtype_t*>(shared_storage.smem_b[0]), 0, 0);
            ...
            while (scheduler.get_next_task(task_info)) {
                ...
                if constexpr (kSharedBF16)
                    mma::sm100::update_instr_desc_with_umma_n(shared_bf16_instr_desc, task_info.get_umma_aligned_valid_m());
                        ...
                        if (not task_info.is_shared()) {
                            ...
                            for (uint32_t k = 0; k < BLOCK_K / 64; ++ k) {
                                ...
                                if (k % kNumSFSubblocks == 0) {
                                    #pragma unroll
                                    for (uint32_t j = 0; j < kNumSFSubblocks; ++ j) {
                                        #pragma unroll
                                        for (uint32_t i = 0; i < SF_BLOCK_M / 128; ++ i) {
                                            mma::sm100::replace_smem_desc_addr(sf_desc,
                                                shared_storage.smem_sfa[stage_idx] + (k + j) * SF_BLOCK_M + i * 128);
                                            cute_utccp_t::copy(sf_desc, kTmemStartColOfSFA + j * (SF_BLOCK_M / 32) + i * 4);
                                        }
                                        mma::sm100::replace_smem_desc_addr(sf_desc,
                                            shared_storage.smem_sfb[stage_idx] + (k + j) * SF_BLOCK_N);
                                ...
                                ptx::SM100_MMA_NVF4_2x1SM_SS::fma(
                                    ...
                                    kTmemStartColOfSFB + (k % kNumSFSubblocks) * (SF_BLOCK_N / 32),
                                    kTmemStartColOfSFA + (k % kNumSFSubblocks) * (SF_BLOCK_M / 32));
                            }
                        } else if constexpr (kSharedBF16) {
                            #pragma unroll
                            for (uint32_t kb = 0; kb < SHARED_BLOCK_K / UMMA_BLOCK_K; ++ kb) {
                                #pragma unroll
                                for (uint32_t k = 0; k < UMMA_BLOCK_K / UMMA_K; ++ k) {
                                    a_desc.lo = mma::sm100::advance_umma_desc_lo<
                                        ...
                                        a_desc_base_lo, kb * UMMA_BLOCK_K * LOAD_BLOCK_M, k * UMMA_K);
                                    b_desc.lo = mma::sm100::advance_umma_desc_lo<
                                        cute::UMMA::Major::K, LOAD_BLOCK_N, kSwizzleBMode, shared_b_dtype_t>(
                                        b_desc_base_lo, kb * UMMA_BLOCK_K * LOAD_BLOCK_N, k * UMMA_K);
                                    ptx::SM100_MMA_F16BF16_2x1SM_SS::fma(
                                        b_desc, a_desc, accum_stage_idx * UMMA_N,
                                        k_block_idx > 0 or kb > 0 or k > 0,
                                        static_cast<uint64_t>(static_cast<uint32_t>(shared_bf16_instr_desc)) << 32);
                                }
                            }
                        } else {
                            #pragma unroll
        ...
        while (scheduler.get_next_task(task_info)) {
            ...
            const float gemm_alpha = task_info.is_shared() ? 1.0f :
                task_info.block_phase == sched::BlockPhase::Linear1 ?
                    (l1_alpha == nullptr ? 1.0f : l1_alpha[task_info.local_expert_idx]) :
            ...
            if (task_info.block_phase == sched::BlockPhase::Linear1 or task_info.block_phase == sched::BlockPhase::SharedLinear1) {
                ...
                for (uint32_t s = 0; s < WG_BLOCK_M / STORE_BLOCK_M; ++ s) {
                    ...
                    for (uint32_t i = 0; i < kNumAtomsPerStore; ++ i) {
                        ...
                        if constexpr (kSharedBF16) {
                            const auto upper = __float22bfloat162_rn(activation_values[i][0]);
                            const auto lower = __float22bfloat162_rn(activation_values[i][1]);
                            const uint32_t row = lane_idx % 8;
                            const uint32_t col = warp_idx_in_wg * 2 + lane_idx / 8;
                            auto smem_ptr = shared_storage.smem_d.l1[epilogue_wg_idx][tma_stage_idx] +
                                (i * ATOM_M + row) * L1_OUT_BLOCK_N +
                                (col ^ row) * (kNumBankGroupBytes / sizeof(shared_dtype_t));
                            ptx::SM90_U32x2_STSM_T<__nv_bfloat162>::copy(upper, lower, smem_ptr);
                            continue;
                        }
                        // Reduce amax (warp-pair-level)
                ...
                for (uint32_t s = 0; s < WG_BLOCK_M / STORE_BLOCK_M; ++ s) {
                    ...
                    for (uint32_t i = 0; i < STORE_BLOCK_M / ATOM_M; ++ i) {
                        ...
                        for (uint32_t v = 0; v < ATOM_M; ++ v)
                            values[v] = __float_as_uint(__uint_as_float(values[v]) * gemm_alpha);
